import {quantityMonth,fuelKey} from './fuelConversion';
...
export function prepareFuelSpreadsheet(plant,month,items){
 ...
 return items.map(item=>{
  ...
  const quantity=get([`${fuel} (kg)`]);const litres=get(['HSD (Ltr)','HSD (litre)','HSD (litres)']);
  ...
  if(plant==='utility'&&item['Fuel Type']&&!['LNG','LPG'].includes(item['Fuel Type']))throw Error('Fuel Type must be LNG or LPG.');
...
export function fuelSampleRows(plant,month,items,rows){
 const fuel=plant==='wider'?'LNG':plant==='utility'?'LNG/LPG':'LPG';const key=fuelKey(plant);
 return items.map((item,i)=>{const out={...item};for(const k of Object.keys(out)){if(/^(LNG|LPG|LNG\/LPG|HSD)\s*\(/i.test(k))delete out[k];}
 out[`${fuel} (${quantityMonth(month)?'kg':'kWh'})`]=rows[i]?.[key]??'';out[`HSD (${quantityMonth(month)?'Ltr':'kWh'})`]=rows[i]?.hsd??'';
 if(plant==='utility'&&quantityMonth(month))out['Fuel Type']=rows[i]?.fuelType||(/LPG/i.test(rows[i]?.equipment)?'LPG':'LNG');
 return out;});
}
