...
export default function ConsumptionBreakdownView({data,colors,children}) {
 const [view,setView]=useState('pie');
 return <div className="enpi-breakdown-view">
  <div className="enpi-breakdown-toggle" role="group" aria-label="Total consumption breakdown chart type">
  ...
  <div className="enpi-breakdown-canvas">
   {!data.length?<p role="status">No total consumption data for this month.</p>:view==='pie'?children:<ExpandableChart>
    <BarChart data={data} margin={{top:32,right:20,left:5,bottom:35}}>
     ...
     <Tooltip cursor={{fill:'#ffffff10'}} contentStyle={{background:'#17102e',border:'1px solid #a78bfa',borderRadius:12,color:'#fff'}} formatter={value=>[Number(value).toLocaleString('en-IN',{maximumFractionDigits:4}),'Total consumption (kWh)']}/>
     <Bar dataKey="value" maxBarSize={65} radius={[8,8,0,0]} isAnimationActive={false}>{data.map((item,index)=><Cell key={`${item.name}-${index}`} fill={colors[(index)%colors.length]}/>)}</Bar>
    </BarChart>
   </ExpandableChart>}
  </div>
 </div>;
