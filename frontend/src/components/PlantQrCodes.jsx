...
export function useEquipmentQrTarget() {
 ...
 useEffect(() => {
  const name = new URLSearchParams(search).get('equipment');
  const node = [...document.querySelectorAll('[data-equipment]')].find(n => n.dataset.equipment === name);
  if (!node) return;
  node.classList.add('equipment-qr-target');
  const frame = requestAnimationFrame(() => node.scrollIntoView({block:'center',inline:'center',behavior:'instant'}));
  return () => {cancelAnimationFrame(frame);node.classList.remove('equipment-qr-target');};
 }, [search]);
}
function QrCard({plant,name}) {
 const [src,setSrc] = useState(''); const [error,setError] = useState(''); const [retry,setRetry] = useState(0);
 const url = new URL(`/${plant}`,window.location.origin); if(name) url.searchParams.set('equipment',name);
...
export default function PlantQrCodes({plant}) {
 ...
 <header><div><h2 id={id}>{plant.replaceAll('-',' ').toUpperCase()} — Equipment QR Codes</h2><p>Download labels. Scan to open the plant and highlight the equipment.</p></div><button type="button" autoFocus onClick={()=>setOpen(false)}>Close ✕</button></header>
