...
export default function NarrowFlatYoYPage() {
  ...
  const fetchYoYData = () => {
    setLoading(true);
    fetch(`http://localhost:5000/api/narrow-flat/yoy?year=${selectedYear}`)
      .then(res => res.json())
      .then(resData => {
      ...
      .catch(err => {
        console.error("Narrow Flat YoY Fetch Error:", err);
        setLoading(false);
      });
  };
  ...
  const getMetricConfig = () => {
    switch (selectedMetric) {
      case 'electricity':
        return { label: 'Electricity Consumption (kWh)', colorPrev: '#94a3b8', colorCurr: '#6366f1', unit: 'kWh' };
      case 'production':
        return { label: 'Production (MT)', colorPrev: '#cbd5e1', colorCurr: '#10b981', unit: 'MT' };
      case 'enpi':
        return { label: 'EnPI Value', colorPrev: '#fcd34d', colorCurr: '#f59e0b', unit: '' };
      default:
        return { label: 'Electricity Consumption (kWh)', colorPrev: '#94a3b8', colorCurr: '#6366f1', unit: 'kWh' };
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 p-6 md:p-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-6 border-b border-slate-200 bg-white p-4 rounded-2xl shadow-sm">
        <div className="flex items-center gap-4">
          ...
          >
            <ArrowLeft size={18} /> Back to Narrow Flat
          </button>
          <div>
            <h1 className="text-xl font-bold text-slate-800">Narrow Flat Facility - YoY Analytics</h1>
      ...
      {/* 5 Fixed Equipment Rows (CRM, CGL, CCL, COMPRESSOR, BOILER) */}
      {loading ? (
        <div className="p-16 text-center text-indigo-600 font-bold animate-pulse">
          Loading Narrow Flat YoY Data for {selectedMetric.toUpperCase()}...
        </div>
      ) : (
