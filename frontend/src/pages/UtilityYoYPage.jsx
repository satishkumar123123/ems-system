...
export default function UtilityYoYPage() {
  ...
  const fetchYoYData = () => {
    setLoading(true);
    fetch(`http://localhost:5000/api/utility/yoy?year=${selectedYear}`)
      .then(res => res.json())
      .then(resData => {
      ...
      .catch(err => {
        console.error("Utility YoY Fetch Error:", err);
        setLoading(false);
      });
  };
  ...
  const getMetricConfig = () => {
    switch (selectedMetric) {
      case 'electricity':
        return { label: 'Electricity Consumption (kWh)', colorPrev: '#94a3b8', colorCurr: '#0284c7', unit: 'kWh' };
      case 'production':
        return { label: 'Production / Output', colorPrev: '#cbd5e1', colorCurr: '#059669', unit: 'Units' };
      case 'enpi':
        return { label: 'EnPI Value', colorPrev: '#fcd34d', colorCurr: '#d97706', unit: '' };
      default:
        return { label: 'Electricity Consumption (kWh)', colorPrev: '#94a3b8', colorCurr: '#0284c7', unit: 'kWh' };
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 p-6 md:p-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-6 border-b border-slate-200 bg-white p-4 rounded-2xl shadow-sm">
        <div className="flex items-center gap-4">
          ...
          >
            <ArrowLeft size={18} /> Back to Utility
          </button>
          <div>
            <h1 className="text-xl font-bold text-slate-800">Utility Facility - YoY Analytics</h1>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-xs tracking-wide transition cursor-pointer ${
              selectedMetric === 'electricity'
                ? 'bg-sky-600 text-white shadow-sky-200 shadow-md'
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-xs tracking-wide transition cursor-pointer ${
              selectedMetric === 'production'
                ? 'bg-teal-600 text-white shadow-teal-200 shadow-md'
                : 'bg-slate-100 text-slate-600 hover:bg-slate-200 border border-slate-300'
            }`}
          >
      ...
      {/* 4 Fixed Equipment Rows (ARP(LNG), Boiler(LPG), Pump house, MEE PLANT(LPG)) */}
      {loading ? (
        <div className="p-16 text-center text-sky-600 font-bold animate-pulse">
          Loading Utility YoY Data for {selectedMetric.toUpperCase()}...
        </div>
      ) : (
              ...
              >
                <div className="flex items-center justify-between mb-4 border-b border-slate-100 pb-3">
                  <div className="flex items-center gap-3">
                    <span className="w-8 h-8 rounded-lg bg-teal-50 border border-teal-200 text-teal-700 font-extrabold flex items-center justify-center text-sm">
                      0{idx + 1}
                    </span>
