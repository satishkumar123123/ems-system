...
export default function UtilityYoYPage() {
  ...
  useEffect(() => {
    ...
    setLoadError('');
    setYoyData([]);
    apiFetch(`${API_BASE_URL}/api/utility/yoy?year=${selectedYear}`, { signal: controller.signal })
      .then(res => res.json())
      .then(data => {
        if (!active) return;
  ...
  const getMetricConfig = () => {
    switch (selectedMetric) {
      ...
      case 'production':
        return { label: 'Production / Output', colorPrev: '#cbd5e1', colorCurr: '#059669', unit: '' };
      case 'totalConsumption':
        return { label: 'Total Consumption (kWh)', colorPrev: '#fcd34d', colorCurr: '#d97706', unit: 'kWh' };
      default:
        return { label: 'Electricity Consumption (kWh)', colorPrev: '#94a3b8', colorCurr: '#0284c7', unit: 'kWh' };
    }
  };
