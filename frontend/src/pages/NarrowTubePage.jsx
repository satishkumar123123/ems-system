...
export default function NarrowTubePage() {
  ...
  const augustFuel=useAugustFuelEquipment('narrow-tube');
  ...
  useEffect(() => {
    ...
    apiFetch(`${API_BASE_URL}/api/narrow-tube?month=${selectedMonth}`, { signal: controller.signal })
      .then(res => res.json())
      .then(data => {
        if (!active) return;
        ...
        if (data?.rows?.length) {
          ...
          setRows(toInputRows('narrow-tube', selectedMonth, merged));
        } else {
          setNoMonthData(true);
        }
      })
  ...
  const handleCellChange = (idx, field, value) => {
    ...
    if (quantityMonth(selectedMonth)||formula) {
      setRows(prev => calculateRows('narrow-tube', prev.map((row, i) => i === idx ? { ...row, [field]: value } : row),formula||defaultFormula(selectedMonth),quantityMonth(selectedMonth)?'quantity-v1':'legacy-kwh'));
      return;
    }
    setRows(prev => {
      const next = [...prev];
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        const parsedData = prepareFuelSpreadsheet('narrow-tube', selectedMonth, XLSX.utils.sheet_to_json(ws, {defval: ''}));
        ...
        if (parsedData.length > 0) {
          ...
          setRows(quantityMonth(selectedMonth)||formula ? calculateRows('narrow-tube', mapped,formula||defaultFormula(selectedMonth),quantityMonth(selectedMonth)?'quantity-v1':'legacy-kwh') : mapped);
          setNoMonthData(false);
  ...
  const handleDownloadSample = () => {
    ...
    const ws = XLSX.utils.json_to_sheet(fuelSampleRows('narrow-tube', selectedMonth, exportData, rows));
  ...
  const executeSave = async (formulaOverride = formula) => {
    ...
    try {
      ...
      const res = await apiFetch(`${API_BASE_URL}/api/narrow-tube/save`, {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          rows: formulaOverride?calculateRows('narrow-tube',rows,formulaOverride,quantityMonth(selectedMonth)?'quantity-v1':'legacy-kwh'):rows,
          totals: {
            ...totals,
            lng: totals.lpg,
      ...
      if (resData.success) {
        ...
        if (resData.data?.rows) setRows(toInputRows('narrow-tube', selectedMonth, resData.data.rows));
        setNoMonthData(false);
  ...
  const saveFormula = next => {
    ...
    setRows(prev=>calculateRows('narrow-tube',prev,next,quantityMonth(selectedMonth)?'quantity-v1':'legacy-kwh'));
          ...
          <table className="equipment-grid" style={{ width: '100%', minWidth: '1300px', fontSize: '12px', textAlign: 'center', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
            ...
            <thead>
              <tr style={{ color: '#020617', fontWeight: '900', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.5px', borderBottom: '2px solid #000' }}>
                ...
                <th style={{ backgroundColor: "#2dd4bf", padding: "12px 6px", borderRight: "2px solid #000" }}> <TotalFormulaControl plant="narrow-tube" month={selectedMonth} formula={formula} onSave={saveFormula} disabled={saving||loading||Boolean(loadError)} saving={saving}/> </th>
                ...
                return (
                  <tr key={idx} data-equipment={r.equipment} style={{ borderBottom: '1px solid #000', fontWeight: 'bold' }}>
                    ...
                    <td style={{ backgroundColor: config.labelBg || '#0284c7', color: config.textCol || '#ffffff', padding: '10px 8px', borderRight: '2px solid #000', textAlign: 'left', fontWeight: '900', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      <Link style={{color: 'inherit', textDecoration: 'underline', textUnderlineOffset: '4px'}} to={`/equipment/narrow-tube?${new URLSearchParams({equipment: r.equipment, month: selectedMonth})}`}>{r.equipment} ↗</Link>
                    </td>
                    ...
                    <td style={{ backgroundColor: COL_COLORS.lpg, padding: '4px', borderRight: '2px solid #000' }}>
                      ...
                      />
                      <FuelSelector plant="narrow-tube" row={r} month={selectedMonth} formula={formula} eligible={augustFuel.allows(r.equipment)} disabled={saving||loading} onChange={value=>handleCellChange(idx,'fuelType',value)}/>
                    </td>
