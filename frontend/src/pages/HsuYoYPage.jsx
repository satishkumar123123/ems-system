...
export default function HsuYoYPage() {
  ...
  const fetchYoYData = () => {
    setLoading(true);
    fetch(`http://localhost:5000/api/hsu/yoy?year=${selectedYear}`)
      .then(res => res.json())
      .then(resData => {
  ...
  const getMetricConfig = () => {
    switch (selectedMetric) {
      ...
      case 'production':
        return { label: 'Production (MT)', colorPrev: '#cbd5e1', colorCurr: '#059669', unit: 'MT' };
      case 'enpi':
        return { label: 'EnPI Value (KwH/MT)', colorPrev: '#fcd34d', colorCurr: '#ea580c', unit: 'KwH/MT' };
      default:
        return { label: 'Electricity Consumption (kWh)', colorPrev: '#94a3b8', colorCurr: '#4f46e5', unit: 'kWh' };
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 p-6 md:p-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-6 border-b border-slate-200 bg-white p-4 rounded-2xl shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800">HSU Facility - YoY Analytics</h1>
