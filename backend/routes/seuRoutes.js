...
router.put('/:year/:id',async(req,res)=>{
 ...
 const plantById=['abpl','wider','hsu','narrow-flat','narrow-tube','utility','wider','wider','wider','wider','utility','utility','utility','utility','utility','narrow-flat','narrow-flat','narrow-flat','narrow-flat','hsu','hsu','hsu','hsu','hsu','narrow-tube','narrow-tube','narrow-tube','narrow-tube','narrow-tube','narrow-tube','narrow-tube'];
 if(!validYear(year)||!/^([1-9]|[12][0-9]|3[01])$/.test(id)||!b||!['higher','lower'].includes(b.direction)||typeof b.unit!=='string'||b.unit.length>40||![b.baseline,b.target,b.tolerance].every(number)||!Array.isArray(b.values)||b.values.length!==12||!b.values.every(number)||typeof b.remark!=='string'||b.remark.length>3000||!Number.isInteger(b.revision)||b.revision<0)return res.status(400).json({error:'Check values, units and remarks. Use nonnegative numbers or blank values.'});
